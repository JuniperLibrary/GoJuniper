...
func main() {
	...
	*/
	...
	/*
		5.仅绑定查询字符串
			ShouldBindQuery 仅将URL查询字符串参数绑定到结构体、完全忽略请求体。当你需要确保POST请求体数据不会
			意外覆盖查询参数--例如在同时接受查询过滤器和jSON请求收的端点中-- 这将非常有用。

			相比之下， ShoudBind 在GET请求中也是用查询绑定 但在 Post 请求中它会首先检查请求体。
			当你明确需要绑定查询字符串而不考虑HTTP方法时，请使用 ShouldBindQuery
	*/
	routeShouldBindQuery := gin.Default()
	routeShouldBindQuery.Any("/testing", startPage)
	routeShouldBindQuery.Run(":8085")
...
type PersonBindQuery struct {
	Name    string `form:"name"`
	Address string `form:"address"`
}

func startPage(c *gin.Context) {
	var person PersonBindQuery
