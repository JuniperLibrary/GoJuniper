...
`internal/channelsx` 实现了三种经典 channel 模式（`channelsx.go`）：
...
对应的代码文件：`internal/channelsx/channelsx.go` / `internal/channelsx/channelsx_test.go`

```
cd /Users/dingchuan/Documents/Repos/GoJuniper && go test ./internal/channelsx/ -v
```
