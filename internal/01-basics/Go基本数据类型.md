...
> ⚠️ **注意**：`int` / `uint` 的**宽度跟随平台**（64 位机器上是 64 位，32 位上是 32 位），不像 Java 的 `int`（固定 32 位）/ `long`（固定 64 位）明确固定。
...
> ⚠️ **注意**：Go **没有** Java 那种隐式数值提升（Java 允许 `int`→`long`、`int`→`double` 等拓宽转换）。Go 中 `int32` 不能直接赋给 `int64`，`int` 和 `int64` 也不是同一类型，必须显式 `int64(x)`。好处是不会"悄悄"丢精度，坏处是写起来啰嗦——但这是刻意的取舍（显式优于隐式）。
...
> ⚠️ **注意（Java 对照）**：Java 里 `ArrayList<T>` 是对象引用，赋值只是复制引用（多个变量指向同一对象），修改元素会互相影响——这和 Go 的 slice 共享底层数组类似。但 Go 的 slice 没有 Java 那种"包装对象"概念，就是 `(ptr, len, cap)` 三个字段的轻量描述符，赋值永远只复制描述符、共享底层数组——这是两种语言内存模型的根本差异，习惯 Java 的人最容易在这里误判"是不是独立副本"（Java 需注意 ArrayList 引用共享；Go 需注意 slice 底层数组共享，用 `copy()` 才独立）。
